...
/**
 * Validate a professional license entry object.
 * @param {Object} lic
 * @returns {Object} errors map for this license entry — empty = valid
 */
function validateProfLicense(lic) {
  const errors = {};
  if (!lic || typeof lic !== 'object') return { _entry: 'Invalid license entry' };

  const VALID_TYPES = new Set(['RN', 'NP', 'Esthetician', 'Other']);
  const VALID_STATUSES = new Set(['Active', 'Suspended', 'Revoked']);

  if (!lic.type || !VALID_TYPES.has(lic.type)) {
    errors.type = 'License type is required';
  }
  if (lic.type === 'Other' && !lic.type_other) {
    errors.type_other = 'Please specify license type';
  }
  if (!lic.number || String(lic.number).trim() === '') {
    errors.number = 'License number is required';
  }
  if (!lic.status || !VALID_STATUSES.has(lic.status)) {
    errors.status = 'License status is required (Active/Suspended/Revoked)';
  }
  if (!lic.expiration || !validateFutureDate(lic.expiration)) {
    errors.expiration = 'License expiration must be in the future';
  }
  if (!lic.license_url || !validateURL(lic.license_url)) {
    errors.license_url = 'Valid public license URL is required';
  }
  return errors;
}
 ...
 * @param {Object} file - { mimetype, size }
...
/** Job titles available in admin employee form */
const JOB_TITLES = [
...
/** Titles that require professional license + insurance sections */
const CLINICAL_TITLES = new Set([
  'Esthetician',
  'Aesthetic Nurse',
  'Aesthetic Nurse Practitioner',
  'Physician',
...
/** Valid professional license types */
const PROF_LICENSE_TYPES = new Set(['RN', 'NP', 'Esthetician', 'Other']);

/** Valid professional license statuses */
const PROF_LICENSE_STATUSES = new Set(['Active', 'Suspended', 'Revoked']);
 ...
 */
function validateOnboarding(form, { requireLicenseInsurance = true } = {}) {
  const errors = {};
  ...
  if (!form.mobile_phone || String(form.mobile_phone).trim() === '') {
    errors.mobile_phone = 'Mobile phone is required';
  } else if (!validatePhone(form.mobile_phone)) {
    errors.mobile_phone = 'Invalid phone number (10-11 digits)';
  }
  ...
  // Tax (W-9) — removed w9_signed_at and w9_exempt_payee_code requirements
  ...
  // Driver's license (mandatory fields — upload path validated server-side during upload)
  if (!form.driver_license_number || String(form.driver_license_number).trim() === '') {
    errors.driver_license_number = "Driver's license number is required";
  }
  if (!form.driver_license_state || !validateState(form.driver_license_state)) {
    errors.driver_license_state = "Driver's license state is required";
  }

  // Professional licenses (JSONB array) — required only for clinical titles
  if (requireLicenseInsurance) {
    const licenses = Array.isArray(form.professional_licenses) ? form.professional_licenses : [];
    if (licenses.length === 0) {
      errors.professional_licenses = 'At least one professional license is required';
    } else {
      licenses.forEach((lic, idx) => {
        const licErrors = validateProfLicense(lic);
        if (Object.keys(licErrors).length > 0) {
          errors[`professional_licenses_${idx}`] = licErrors;
        }
      });
    }
  } else if (Array.isArray(form.professional_licenses) && form.professional_licenses.length > 0) {
    // If submitted voluntarily, still validate format
    form.professional_licenses.forEach((lic, idx) => {
      const licErrors = validateProfLicense(lic);
      if (Object.keys(licErrors).length > 0) {
        errors[`professional_licenses_${idx}`] = licErrors;
      }
  ...
  // Insurance — coverage amounts required for clinical titles
  if (requireLicenseInsurance) {
    if (!form.prof_liability_per_occurrence) {
      errors.prof_liability_per_occurrence = 'Per-occurrence coverage amount is required';
    }
    if (!form.prof_liability_aggregate) {
      errors.prof_liability_aggregate = 'Aggregate coverage amount is required';
    }
  }
